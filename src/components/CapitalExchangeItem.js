...
@observer
export default class CapitalExchangeItem extends RNComponent {
    ...
    re_getFundList = function (data) {
        ...
        console.log('获取资金往来记录', data)
        this.fundLists = data.dataMap.recordList || []
    ...
    goToFundDetail = (item,fundType) => {
        let fundStatus = item.done == "SUCCESS" ? '成功' : '失败'

        this.$router.push('CapitalExchangeDetail', {item,fundType,fundStatus})
