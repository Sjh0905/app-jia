...
import styles from "../style/TransactionSellStyle";
...
@observer
export default class TransactionSell extends RNComponent {
    ...
    //确认购买弹窗数据
    @observable sellCommitToastData = {}
    //确认购买弹窗
    @observable sellCommitToastOpen = false
    ...
    clickToSell = ()=>{
        ...
        this.sellCommitToastData = Object.assign({},this.sellItem);
        this.sellCommitToastData.amount = this.inputNum
        this.sellCommitToastData.orderId = this.sellCommitToastData.id
    ...
    @action
    clickConfirmBtn = ()=>{
        console.log('this is c2c to sell params',{
    ...
    bindInputNum =  (inputNum) =>{
        ...
        this.inputCNY = this.$globalFunc.accFixed(this.$globalFunc.accMul(inputNum,this.sellItem.price),2)
    }
    ...
    bindInputCNY = (inputCNY)=> {
        ...
        this.inputNum = this.$globalFunc.accFixed(this.$globalFunc.accDiv(inputCNY,this.sellItem.price),0)
    }
    ...
    renderInput = ()=>{
        ...
        return <View style={styles.buyInputBox}>
            ...
            <View style={[SignBaseStyle.inputItemBox,styles.itemInput,{
                ...
                <Text style={styles.inputField}>收款方式：</Text>
                <Text style={styles.inputBank}>银行转账</Text>
                <Text style={styles.inputTips}>（须本人账户支付）</Text>
            </View>
            <View style={styles.subBtnBox}>
                <BaseButton
                    onPress={this.clickToSell}
                    style={[SignBaseStyle.button,{backgroundColor:StyleConfigs.btnRed,width:'94%'}]}
        ...
        return (
            ...
            >
                <View style={styles.rowV1}>
                    <Text  allowFontScaling={false} style={[styles.size14,BaseStyles.textGreen]}>买入</Text>
                </View>
                ...
                <View style={styles.rowV3}>
                    <Text  allowFontScaling={false} style={[styles.size14,BaseStyles.textWhite]}>{limtMoney}</Text>
                </View>

                <View style={styles.rowV4}>
                    <Text  allowFontScaling={false} style={[styles.size14,BaseStyles.textGreen]}>{this.$globalFunc.accFixed(rowData.price || 0,2)}</Text>
                </View>
            </TouchableOpacity>
        )
    }
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                ...
                {
                    this.sellCommitToastOpen &&
                    <OrderConfirmPopups
                        alertBoxStyle={{justifyContent:'flex-start'}}
                        style={{marginTop:getHeight(150)}}
                        data={{order:this.sellCommitToastData,userinfo:this.sellCommitToastData}}
                        type={'SELL_ORDER'}
                        close={false}
                        title={'卖出USDT'}
